...
export const BroadcastRecordingForm = () => {
    ...
    const fetchItem = async () => {
        try {
            const response = await api.get(`/api/broadcast-recording/${id}`);
            ...
            toast.error('Ошибка загрузки записи эфира');
            navigate('/admin/broadcast-recordings');
        }
    };
    ...
    const moveContentItem = (fromIndex: number, toIndex: number) => {
        setFormData(prev => {
            if (toIndex < 0 || toIndex >= prev.content.length) {
                return prev;
            }
            const newContent = [...prev.content];
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (id) {
                await api.put(`/api/broadcast-recording/${id}`, payload);
                toast.success('Запись эфира обновлена');
            } else {
                await api.post('/api/broadcast-recording', payload);
                toast.success('Запись эфира создана');
            }
            navigate('/admin/broadcast-recordings');
        } catch (error) {
        ...
        <AdminLayout>
            <div className="max-w-6xl mx-auto space-y-6">
                <div className="flex items-center gap-4">
                    <button
                        onClick={() => navigate('/admin/broadcast-recordings')}
                        className="p-2 hover:bg-gray-100 rounded-lg transition-colors"
                    >
                        <ArrowLeft size={24} />
                    </button>
                    <h1 className="text-3xl font-bold text-gray-900">
                        {id ? 'Редактировать запись эфира' : 'Добавить запись эфира'}
                    </h1>
                </div>
                                ...
                                <div key={index} className="p-4 border border-gray-300 rounded-lg bg-gray-50">
                                    ...
                                    <div className="space-y-3">
                                        <div className="flex flex-col gap-2">
                                            ...
                                            <select
                                                value={item.type}
                                                onChange={(e) => handleTypeChange(index, e.target.value as ContentItem['type'])}
                                                className="w-full p-2 rounded-md border border-gray-300"
                                            >
                                        ...
                                        {item.type === 'video' && (
                                            <>
                                                <MyInput
                                                    ...
                                                    type="text"
                                                    value={item.video?.mainUrl || ''}
                                                    onChange={(e) => handleVideoChange(index, 'mainUrl', e.target.value)}
                                                    placeholder="https://..."
                                                />
                                                <MyInput
                                                    label="Резервная ссылка на видео"
                                                    type="text"
                                                    value={item.video?.reserveUrl || ''}
                                                    onChange={(e) => handleVideoChange(index, 'reserveUrl', e.target.value)}
                                                    placeholder="https://..."
                                                />
                                                <MyInput
