...
export const RelationshipWorkshopForm = () => {
    const navigate = useNavigate();
    const { id } = useParams();
    const [loading, setLoading] = useState(false);
    const [formData, setFormData] = useState<FormData>({
        title: '', shortDescription: '', imageUrl: '', accessType: 'free', starsRequired: 0, price: 0, duration: 0, order: 0, allowRepeatBonus: false, location: 'bottom', redirectToPage: '', visibility: true, content: [],
    });
    ...
    const fetchItem = async () => {
        try {
            const response = await api.get(`/api/relationship-workshop/${id}`);
    ...
    const handleContentChange = (index: number, field: 'text' | 'image', value: string) => {
        setFormData(prev => { const newContent = [...prev.content]; newContent[index] = { ...newContent[index], [field]: value }; return { ...prev, content: newContent }; });
    };
    ...
    const removeContentItem = (index: number) => {
        setFormData(prev => { const newContent = [...prev.content]; newContent.splice(index, 1); return { ...prev, content: newContent }; });
    };
        ...
        <AdminLayout>
            <div className="max-w-6xl mx-auto space-y-6">
                <div className="flex items-center gap-4">
                    <button onClick={() => navigate('/admin/relationship-workshop')} className="p-2 hover:bg-gray-100 rounded-lg transition-colors"><ArrowLeft size={24} /></button>
                    <h1 className="text-3xl font-bold text-gray-900">{id ? 'Редактировать запись' : 'Добавить запись'} - Мастерская отношений</h1>
                </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <div className="flex gap-3 justify-end bg-white rounded-lg shadow-sm p-6">
                        <button type="button" onClick={() => navigate('/admin/relationship-workshop')} className="px-6 py-2 border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors">Отмена</button>
