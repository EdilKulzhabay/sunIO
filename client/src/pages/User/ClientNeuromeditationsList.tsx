...
const CLIENT_PATH = "/client/neuromeditations";
const CONTENT_TYPE = "neuromeditations";
const TITLE = "Нейромедитации";

export const ClientNeuromeditationsList = () => {
    ...
    const fetchContent = async () => {
        ...
        try {
            const responseDinamycContent = await api.get("/api/dynamic-content/name/meditaions-list");
    ...
    const fetchItems = async () => {
        try {
            ...
            console.error("Ошибка загрузки нейромедитаций:", error);
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <div>
            <UserLayout>
                ...
                {dinamycContent ? <div className="px-4 mt-2" dangerouslySetInnerHTML={{ __html: dinamycContent }} /> : null}
                    ...
                    <div ref={cardsContainerRef} className="flex overflow-x-auto gap-4 scrollbar-hide pl-4 pr-4 -mx-4" style={{ scrollbarWidth: "none" }}>
                        {topItems.length > 0 &&
                            topItems.map((item: any) => (
                                <div key={item._id} data-card className="flex-shrink-0 w-[44vw] sm:w-[35vw] lg:w-[25vw] h-[210px] sm:h-[275px] lg:h-[330px]">
                                    <MiniVideoCard
                                        title={item.title}
                                        image={item.imageUrl}
                                        link={item.redirectToPage?.trim() || `${CLIENT_PATH}/${item._id}`}
                                        progress={progresses[item._id] || 0}
                                        accessType={hasAccessToContent(item._id) ? "free" : item.accessType}
                                        onLockedClick={
                                            hasAccessToContentSubscription() ? undefined : item.accessType !== "free" ? () => handleLockedClickSubscription(item) : undefined
                                        }
                                        duration={item?.duration || 0}
                                        starsRequired={item?.starsRequired || 0}
                                    />
                                </div>
                            ))}
                    </div>
                        ...
                        {bottomItems.length > 0 &&
                            bottomItems.map((item: any) => (
                                <VideoCard
                                    ...
                                    accessType={hasAccessToContent(item._id) ? "free" : item.accessType}
                                    progress={progresses[item._id] || 0}
                                    onLockedClick={hasAccessToContent(item._id) ? undefined : item.accessType !== "free" ? () => handleLockedClick(item) : undefined}
                                    starsRequired={item?.starsRequired || 0}
                                    duration={item?.duration || 0}
                                />
                            ))}
