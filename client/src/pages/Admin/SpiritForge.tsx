...
export const SpiritForgeAdmin = () => {
    ...
    const fetchItems = async () => {
        try { const response = await api.get('/api/spirit-forge?admin=1'); setItems(response.data.data); }
        catch (error: any) { toast.error('Ошибка загрузки данных'); }
    };

    const handleCreate = () => { navigate('/admin/spirit-forge/create'); };
    const handleEdit = (item: any) => { navigate(`/admin/spirit-forge/edit/${item._id}`); };
    const handleDelete = async (item: any) => {
        if (!confirm('Вы уверены, что хотите удалить эту запись?')) return;
        try { await api.delete(`/api/spirit-forge/${item._id}`); toast.success('Запись удалена'); fetchItems(); }
        ...
        <AdminLayout>
            <div className="space-y-6">
                <div className="flex justify-between items-center">
                    <h1 className="text-3xl font-bold text-gray-900">Кузница Духа</h1>
