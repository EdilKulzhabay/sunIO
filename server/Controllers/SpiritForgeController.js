...
export const create = async (req, res) => {
    try {
        ...
        const spiritForge = new SpiritForge({
            title,
        ...
        await spiritForge.save();

        await addAdminAction(user._id, `Создал(а) запись в Кузнице Духа: "${title}"`);
        ...
        console.log("Ошибка в SpiritForgeController.create:", error);
        res.status(500).json({
            success: false,
...
export const getAll = async (req, res) => {
    try {
        ...
        const items = await SpiritForge.find(filter).sort({ order: 1, createdAt: -1 });
        ...
        console.log("Ошибка в SpiritForgeController.getAll:", error);
        res.status(500).json({
            success: false,
...
export const getById = async (req, res) => {
    try {
        ...
        const item = await SpiritForge.findById(id);
        ...
        console.log("Ошибка в SpiritForgeController.getById:", error);
        res.status(500).json({
            success: false,
...
export const update = async (req, res) => {
    try {
        ...
        const item = await SpiritForge.findByIdAndUpdate(
            id,
            updateData,
        ...
        await addAdminAction(user._id, `Обновил(а) запись в Кузнице Духа: "${item.title}"`);
        ...
        console.log("Ошибка в SpiritForgeController.update:", error);
        res.status(500).json({
            success: false,
...
export const remove = async (req, res) => {
    try {
        ...
        const item = await SpiritForge.findByIdAndDelete(id);
        ...
        await addAdminAction(user._id, `Удалил(а) запись из Кузницы Духа: "${item.title}"`);
        ...
        console.log("Ошибка в SpiritForgeController.remove:", error);
        res.status(500).json({
            success: false,
