...
export const create = async (req, res) => {
    try {
        ...
        await item.save();
        await addAdminAction(user._id, `Создал(а) запись в Разборы - Отношения: "${title}"`);

        res.status(201).json({ success: true, data: item, message: "Запись успешно создана" });
    } catch (error) {
        console.log("Ошибка в AnalysisRelationshipsController.create:", error);
...
export const getAll = async (req, res) => {
    try {
        ...
        console.log("Ошибка в AnalysisRelationshipsController.getAll:", error);
...
export const getById = async (req, res) => {
    try {
        const { id } = req.params;
        const item = await AnalysisRelationships.findById(id);
        ...
        console.log("Ошибка в AnalysisRelationshipsController.getById:", error);
...
export const update = async (req, res) => {
    try {
        ...
        const item = await AnalysisRelationships.findByIdAndUpdate(id, updateData, { new: true, runValidators: true });

        if (!item) {
            return res.status(404).json({ success: false, message: "Запись не найдена" });
        }

        await addAdminAction(user._id, `Обновил(а) запись в Разборы - Отношения: "${item.title}"`);
        res.json({ success: true, data: item, message: "Запись успешно обновлена" });
    } catch (error) {
        console.log("Ошибка в AnalysisRelationshipsController.update:", error);
...
export const remove = async (req, res) => {
    try {
        const user = req.user;
        const { id } = req.params;
        const item = await AnalysisRelationships.findByIdAndDelete(id);

        if (!item) {
            return res.status(404).json({ success: false, message: "Запись не найдена" });
        }

        await addAdminAction(user._id, `Удалил(а) запись из Разборы - Отношения: "${item.title}"`);
        res.json({ success: true, message: "Запись успешно удалена" });
    } catch (error) {
        console.log("Ошибка в AnalysisRelationshipsController.remove:", error);
