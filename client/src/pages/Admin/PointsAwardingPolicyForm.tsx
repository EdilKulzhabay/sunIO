...
export const PointsAwardingPolicyForm = () => {
    const { id } = useParams();
    ...
    const fetchPolicy = async () => {
        try {
            const response = await api.get(`/api/points-awarding-policy/${id}`);
            ...
            navigate('/admin/points-awarding-policy');
        }
    };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            if (isEdit) {
                await api.put(`/api/points-awarding-policy/${id}`, formData);
                toast.success('Политика обновлена');
            } else {
                await api.post('/api/points-awarding-policy', formData);
        ...
        <AdminLayout>
            <div className="space-y-6">
                <div className="flex items-center gap-4">
                    <button
                        onClick={() => navigate('/admin/points-awarding-policy')}
                    ...
                    <h1 className="text-3xl font-bold text-gray-900">
                        {isEdit ? 'Редактировать политику' : 'Создать политику'}
                    </h1>
                </div>
