...
export const create = async (req, res) => {
    try {
        ...
        const consciousnessLibrary = new ConsciousnessLibrary({
            title,
        ...
        await consciousnessLibrary.save();

        await addAdminAction(user._id, `Создал(а) запись в Библиотеке сознания: "${title}"`);

        res.status(201).json({
            success: true,
            data: consciousnessLibrary,
            message: "Запись успешно создана",
        });
    } catch (error) {
        console.log("Ошибка в ConsciousnessLibraryController.create:", error);
        res.status(500).json({
            success: false,
...
export const getAll = async (req, res) => {
    try {
        ...
        const items = await ConsciousnessLibrary.find(filter).sort({ order: 1, createdAt: -1 });
        ...
        console.log("Ошибка в ConsciousnessLibraryController.getAll:", error);
        res.status(500).json({
            success: false,
...
export const getById = async (req, res) => {
    try {
        ...
        const item = await ConsciousnessLibrary.findById(id);
        ...
        console.log("Ошибка в ConsciousnessLibraryController.getById:", error);
        res.status(500).json({
            success: false,
...
export const update = async (req, res) => {
    try {
        ...
        const item = await ConsciousnessLibrary.findByIdAndUpdate(
            id,
            updateData,
        ...
        await addAdminAction(user._id, `Обновил(а) запись в Библиотеке сознания: "${item.title}"`);
        ...
        console.log("Ошибка в ConsciousnessLibraryController.update:", error);
        res.status(500).json({
            success: false,
...
export const remove = async (req, res) => {
    try {
        ...
        const item = await ConsciousnessLibrary.findByIdAndDelete(id);
        ...
        await addAdminAction(user._id, `Удалил(а) запись из Библиотеки сознания: "${item.title}"`);
        ...
        console.log("Ошибка в ConsciousnessLibraryController.remove:", error);
        res.status(500).json({
            success: false,
