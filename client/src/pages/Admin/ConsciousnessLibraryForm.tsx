...
export const ConsciousnessLibraryForm = () => {
    ...
    const fetchItem = async () => {
        try {
            const response = await api.get(`/api/consciousness-library/${id}`);
        ...
        } catch (error) { toast.error('Ошибка загрузки данных'); navigate('/admin/consciousness-library'); }
    };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (id) { await api.put(`/api/consciousness-library/${id}`, payload); toast.success('Запись обновлена'); } else { await api.post('/api/consciousness-library', payload); toast.success('Запись создана'); }
            navigate('/admin/consciousness-library');
    ...
    return (
        <AdminLayout>
            <div className="max-w-6xl mx-auto space-y-6">
                <div className="flex items-center gap-4"><button onClick={() => navigate('/admin/consciousness-library')} className="p-2 hover:bg-gray-100 rounded-lg transition-colors"><ArrowLeft size={24} /></button><h1 className="text-3xl font-bold text-gray-900">{id ? 'Редактировать запись' : 'Добавить запись'} - Библиотека сознания</h1></div>
