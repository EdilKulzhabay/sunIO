...
export const AnalysisRelationshipsAdmin = () => {
    ...
    const handleCreate = () => { navigate('/admin/analysis-relationships/create'); };
    const handleEdit = (item: any) => { navigate(`/admin/analysis-relationships/edit/${item._id}`); };
    const handleDelete = async (item: any) => {
        if (!confirm('Вы уверены, что хотите удалить эту запись?')) return;
        try { await api.delete(`/api/analysis-relationships/${item._id}`); toast.success('Запись удалена'); fetchItems(); }
        ...
        <AdminLayout>
            <div className="space-y-6">
                <div className="flex justify-between items-center">
                    <h1 className="text-3xl font-bold text-gray-900">Разборы - Отношения</h1>
