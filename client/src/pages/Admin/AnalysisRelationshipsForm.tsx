...
export const AnalysisRelationshipsForm = () => {
    ...
    const fetchItem = async () => {
        try {
            const response = await api.get(`/api/analysis-relationships/${id}`);
        ...
        } catch (error) { toast.error('Ошибка загрузки данных'); navigate('/admin/analysis-relationships'); }
    };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (id) { await api.put(`/api/analysis-relationships/${id}`, payload); toast.success('Запись обновлена'); } else { await api.post('/api/analysis-relationships', payload); toast.success('Запись создана'); }
            navigate('/admin/analysis-relationships');
    ...
    return (
        <AdminLayout>
            <div className="max-w-6xl mx-auto space-y-6">
                <div className="flex items-center gap-4"><button onClick={() => navigate('/admin/analysis-relationships')} className="p-2 hover:bg-gray-100 rounded-lg transition-colors"><ArrowLeft size={24} /></button><h1 className="text-3xl font-bold text-gray-900">{id ? 'Редактировать запись' : 'Добавить запись'} - Разборы - Отношения</h1></div>
                <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <div className="flex gap-3 justify-end bg-white rounded-lg shadow-sm p-6"><button type="button" onClick={() => navigate('/admin/analysis-relationships')} className="px-6 py-2 border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors">Отмена</button><MyButton text={loading ? 'Сохранение...' : 'Сохранить'} type="submit" disabled={loading} /></div>
                </form>
            </div>
        </AdminLayout>
    );
