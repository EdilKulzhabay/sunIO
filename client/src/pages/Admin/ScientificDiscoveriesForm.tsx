...
const ADMIN_PATH = '/admin/scientific-discoveries';

export const ScientificDiscoveriesForm = () => {
    ...
    const fetchItem = async () => {
        ...
        } catch (error) { toast.error('Ошибка загрузки данных'); navigate(ADMIN_PATH); }
    };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (id) { await api.put(`${API_PATH}/${id}`, payload); toast.success('Запись обновлена'); } else { await api.post(API_PATH, payload); toast.success('Запись создана'); }
            navigate(ADMIN_PATH);
    ...
    return (
        <AdminLayout>
            <div className="max-w-6xl mx-auto space-y-6">
                <div className="flex items-center gap-4"><button onClick={() => navigate(ADMIN_PATH)} className="p-2 hover:bg-gray-100 rounded-lg transition-colors"><ArrowLeft size={24} /></button><h1 className="text-3xl font-bold text-gray-900">{id ? 'Редактировать научное открытие' : 'Добавить научное открытие'}</h1></div>
