...
interface ContentItem {
    ...
    text?: string; image?: string; linkButton?: LinkButtonValue | null;
}
...
const ADMIN_PATH = '/admin/parables-of-life';
const TITLE = 'притчу';

export const ParablesOfLifeForm = () => {
    ...
    const fetchItem = async () => {
        try {
            ...
            const mappedContent: ContentItem[] = (data.content || []).map((item: any) => {
                ...
                return {
                    ...
                    linkButton: item?.linkButton?.linkButtonText || item?.linkButton?.linkButtonLink ? { linkButtonText: item?.linkButton?.linkButtonText || null, linkButtonLink: item?.linkButton?.linkButtonLink || null, linkButtonType: item?.linkButton?.linkButtonType || 'internal' } : null,
                };
            });
        ...
        } catch (error) { toast.error(`Ошибка загрузки ${TITLE}`); navigate(ADMIN_PATH); }
    };

    const handleVideoChange = (index: number, field: 'mainUrl' | 'reserveUrl' | 'duration' | 'points', value: string | number) => { setFormData(prev => { const newContent = [...prev.content]; newContent[index] = { ...newContent[index], video: { ...newContent[index].video!, [field]: value } }; return { ...prev, content: newContent }; }); };
    ...
    const addContentItem = (type: ContentItem['type']) => { setFormData(prev => { const base = type === 'linkButton' ? { type: 'linkButton' as const, linkButton: { linkButtonText: null, linkButtonLink: null, linkButtonType: 'internal' as const } } : { type, video: { mainUrl: '', reserveUrl: '', duration: 0, points: 0 }, text: '', image: '' }; return { ...prev, content: [...prev.content, base] }; }); };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (id) { await api.put(`${API_PATH}/${id}`, payload); toast.success(`${TITLE.charAt(0).toUpperCase() + TITLE.slice(1)} обновлена`); } else { await api.post(API_PATH, payload); toast.success(`${TITLE.charAt(0).toUpperCase() + TITLE.slice(1)} создана`); }
            navigate(ADMIN_PATH);
    ...
    return (
        <AdminLayout>
            <div className="max-w-6xl mx-auto space-y-6">
                <div className="flex items-center gap-4"><button onClick={() => navigate(ADMIN_PATH)} className="p-2 hover:bg-gray-100 rounded-lg transition-colors"><ArrowLeft size={24} /></button><h1 className="text-3xl font-bold text-gray-900">{id ? 'Редактировать притчу' : 'Добавить притчу'}</h1></div>
                <form onSubmit={handleSubmit} className="space-y-6">
                    <div className="bg-white rounded-lg shadow-sm p-6 space-y-4">
                        ...
                        <div className="flex flex-col gap-2"><label className="text-sm font-medium">Ссылка перехода</label><select value={formData.redirectToPage} onChange={(e) => setFormData({ ...formData, redirectToPage: e.target.value })} className="w-full p-2 rounded-md border border-gray-300">{REDIRECT_TO_PAGE_OPTIONS.map((opt) => (<option key={opt.value || 'empty'} value={opt.value}>{opt.title}</option>))}</select><p className="text-xs text-gray-500">Если выбрана страница — при нажатии на карточку откроется она вместо страницы контента</p></div>
                    ...
                    <div className="flex gap-3 justify-end bg-white rounded-lg shadow-sm p-6"><button type="button" onClick={() => navigate(ADMIN_PATH)} className="px-6 py-2 border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors">Отмена</button><MyButton text={loading ? 'Сохранение...' : 'Сохранить'} type="submit" disabled={loading} /></div>
                </form>
            </div>
        </AdminLayout>
    );
