...
const API_PATH = '/api/neuromeditations';
const ADMIN_PATH = '/admin/neuromeditations';

export const NeuromeditationsForm = () => {
        ...
        try {
            ...
            const mappedContent: ContentItem[] = (data.content || []).map((item: any) => {
                ...
                return {
                    ...
                    image: item?.image || '',
                    linkButton:
                        item?.linkButton?.linkButtonText || item?.linkButton?.linkButtonLink
                            ? {
            ...
            toast.error('Ошибка загрузки нейромедитации');
            navigate(ADMIN_PATH);
        }
    };
    ...
    const handleVideoChange = (
        index: number,
        field: 'mainUrl' | 'reserveUrl' | 'duration' | 'points',
        value: string | number
    ) => {
        setFormData((prev) => {
        ...
        setFormData((prev) => {
            ...
            if (newType === 'linkButton') {
                newContent[index] = {
                    type: 'linkButton',
                    linkButton:
                        newContent[index].linkButton ??
                        ({
                            linkButtonText: null,
                            linkButtonLink: null,
                            linkButtonType: 'internal',
                        } as LinkButtonValue),
                };
            } else {
                newContent[index] = {
                    ...
                    video:
                        newType === 'video'
                            ? (newContent[index].video ?? { mainUrl: '', reserveUrl: '', duration: 0, points: 0 })
                            : undefined,
                ...
                type === 'linkButton'
                    ? {
                          ...
                          linkButton: {
                              linkButtonText: null,
                              linkButtonLink: null,
                              linkButtonType: 'internal' as const,
                          },
                      }
                    : { type, video: { mainUrl: '', reserveUrl: '', duration: 0, points: 0 }, text: '', image: '' };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (id) {
                ...
                toast.success('Нейромедитация обновлена');
            } else {
                await api.post(API_PATH, payload);
                toast.success('Нейромедитация создана');
            }
            navigate(ADMIN_PATH);
        } catch {
        ...
        <AdminLayout>
            <div className="max-w-6xl mx-auto space-y-6">
                <div className="flex items-center gap-4">
                    <button type="button" onClick={() => navigate(ADMIN_PATH)} className="p-2 hover:bg-gray-100 rounded-lg transition-colors">
                        <ArrowLeft size={24} />
                    </button>
                    <h1 className="text-3xl font-bold text-gray-900">{id ? 'Редактировать нейромедитацию' : 'Добавить нейромедитацию'}</h1>
                </div>
                <form onSubmit={handleSubmit} className="space-y-6">
                    <div className="bg-white rounded-lg shadow-sm p-6 space-y-4">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <MyInput label="Порядок" type="text" value={formData.order.toString()} onChange={(e) => setFormData({ ...formData, order: parseInt(e.target.value) || 0 })} placeholder="0" />
                            <div className="flex flex-col gap-2">
                        ...
                        <RedirectToPageSelector value={formData.redirectToPage} onChange={(val) => setFormData({ ...formData, redirectToPage: val })} />
                        <div className="-mt-2">
                    ...
                    <div className="bg-white rounded-lg shadow-sm p-6 space-y-6">
                        ...
                        <div className="space-y-4">
                                ...
                                <div key={index} className="p-4 border border-gray-300 rounded-lg bg-gray-50">
                                    <div className="flex items-center justify-between mb-3">
                                        ...
                                        <div className="flex items-center gap-2">
                                            <button type="button" onClick={() => moveContentItem(index, index - 1)} disabled={index === 0} className="px-2 py-1 text-sm border border-gray-300 rounded hover:bg-gray-100 disabled:opacity-50 disabled:cursor-not-allowed">
                                                Вверх
                                            </button>
                                            <button type="button" onClick={() => moveContentItem(index, index + 1)} disabled={index === formData.content.length - 1} className="px-2 py-1 text-sm border border-gray-300 rounded hover:bg-gray-100 disabled:opacity-50 disabled:cursor-not-allowed">
                                                Вниз
                                            </button>
                                            <button type="button" onClick={() => removeContentItem(index)} className="p-1 text-red-600 hover:bg-red-50 rounded transition-colors">
                                                <Trash2 size={16} />
                                            </button>
                                        </div>
                                    </div>
                            ...
                            {formData.content.length === 0 && (
                                <div className="text-center py-8 text-gray-500 border-2 border-dashed border-gray-300 rounded-lg">Нет элементов контента. Нажмите «Добавить элемент», чтобы начать.</div>
                            )}
                        </div>
                    ...
                    <div className="flex gap-3 justify-end bg-white rounded-lg shadow-sm p-6">
                        <button type="button" onClick={() => navigate(ADMIN_PATH)} className="px-6 py-2 border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors">
                            Отмена
                        </button>
