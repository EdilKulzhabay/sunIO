...
export const create = async (req, res) => {
    try {
                  ...
                  if (item?.video?.mainUrl || item?.video?.reserveUrl) {
                      return {
                          video: {
                              ...
                              duration: Number.isFinite(item?.video?.duration)
                                  ? item.video.duration
                                  : 0,
        ...
        const item = new Neuromeditation({
            title,
        ...
        await addAdminAction(user._id, `Создал(а) нейромедитацию: "${title}"`);
        ...
        console.log("Ошибка в NeuromeditationController.create:", error);
        res.status(500).json({
            success: false,
...
export const getAll = async (req, res) => {
    try {
        ...
        const items = await Neuromeditation.find(filter).sort({ order: 1, createdAt: -1 });
        ...
        const itemsWithCalculatedDuration = items.map((item) => {
            const totalDuration = Array.isArray(item.content)
                ? item.content.reduce((sum, contentItem) => {
                      const d = Number.isFinite(contentItem?.video?.duration)
                          ? contentItem.video.duration
                          : 0;
                      return sum + d;
                  }, 0)
                : 0;
        ...
        console.log("Ошибка в NeuromeditationController.getAll:", error);
        res.status(500).json({
            success: false,
...
export const getById = async (req, res) => {
    try {
        ...
        const item = await Neuromeditation.findById(id);
        ...
        console.log("Ошибка в NeuromeditationController.getById:", error);
        res.status(500).json({
            success: false,
...
export const update = async (req, res) => {
    try {
        ...
        if (updateData.duration !== undefined) {
            updateData.duration = Number.isFinite(updateData.duration) ? updateData.duration : 0;
        }
        if (updateData.order !== undefined) {
            updateData.order = Number.isFinite(updateData.order) ? updateData.order : 0;
        }

        const item = await Neuromeditation.findByIdAndUpdate(id, updateData, {
            new: true,
            runValidators: true,
        });
        ...
        await addAdminAction(user._id, `Обновил(а) нейромедитацию: "${item.title}"`);
        ...
        console.log("Ошибка в NeuromeditationController.update:", error);
        res.status(500).json({
            success: false,
...
export const remove = async (req, res) => {
    try {
        ...
        const item = await Neuromeditation.findByIdAndDelete(id);
        ...
        await addAdminAction(user._id, `Удалил(а) нейромедитацию: "${item.title}"`);
        ...
        console.log("Ошибка в NeuromeditationController.remove:", error);
        res.status(500).json({
            success: false,
