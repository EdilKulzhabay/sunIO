...
export const ProductCatalogAdmin = () => {
    ...
    const fetchItems = async () => {
        try { const response = await api.get('/api/product-catalog?admin=1'); setItems(response.data.data); }
        catch (error: any) { toast.error('Ошибка загрузки данных'); }
    };

    const handleCreate = () => { navigate('/admin/product-catalog/create'); };
    const handleEdit = (item: any) => { navigate(`/admin/product-catalog/edit/${item._id}`); };
    const handleDelete = async (item: any) => {
        if (!confirm('Вы уверены, что хотите удалить эту запись?')) return;
        try { await api.delete(`/api/product-catalog/${item._id}`); toast.success('Запись удалена'); fetchItems(); }
        ...
        <AdminLayout>
            <div className="space-y-6">
                <div className="flex justify-between items-center">
                    <h1 className="text-3xl font-bold text-gray-900">Каталог платных продуктов</h1>
