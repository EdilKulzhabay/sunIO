...
export const create = async (req, res) => {
    try {
        ...
        await addAdminAction(user._id, `Создал(а) запись в Каталоге продуктов: "${title}"`);
        ...
        console.log("Ошибка в ProductCatalogController.create:", error);
        res.status(500).json({
            success: false,
...
export const getAll = async (req, res) => {
    try {
        ...
        const items = await ProductCatalog.find(filter).sort({ order: 1, createdAt: -1 });
        ...
        console.log("Ошибка в ProductCatalogController.getAll:", error);
        res.status(500).json({
            success: false,
...
export const getById = async (req, res) => {
    try {
        ...
        console.log("Ошибка в ProductCatalogController.getById:", error);
        res.status(500).json({
            success: false,
...
export const update = async (req, res) => {
    try {
        ...
        const item = await ProductCatalog.findByIdAndUpdate(
            id,
            updateData,
        ...
        await addAdminAction(user._id, `Обновил(а) запись в Каталоге продуктов: "${item.title}"`);
        ...
        console.log("Ошибка в ProductCatalogController.update:", error);
        res.status(500).json({
            success: false,
...
export const remove = async (req, res) => {
    try {
        ...
        await addAdminAction(user._id, `Удалил(а) запись из Каталога продуктов: "${item.title}"`);
        ...
        console.log("Ошибка в ProductCatalogController.remove:", error);
        res.status(500).json({
            success: false,
