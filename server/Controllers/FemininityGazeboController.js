...
export const create = async (req, res) => {
    try {
        ...
        await femininityGazebo.save();

        await addAdminAction(user._id, `Создал(а) запись в Беседке женственности: "${title}"`);

        res.status(201).json({
            success: true,
            data: femininityGazebo,
            message: "Запись успешно создана",
        });
    } catch (error) {
        console.log("Ошибка в FemininityGazeboController.create:", error);
        res.status(500).json({
            success: false,
...
export const getById = async (req, res) => {
    try {
        ...
        const item = await FemininityGazebo.findById(id);
        ...
        console.log("Ошибка в FemininityGazeboController.getById:", error);
        res.status(500).json({
            success: false,
...
export const update = async (req, res) => {
    try {
        ...
        const item = await FemininityGazebo.findByIdAndUpdate(
            id,
            updateData,
        ...
        await addAdminAction(user._id, `Обновил(а) запись в Беседке женственности: "${item.title}"`);
        ...
        console.log("Ошибка в FemininityGazeboController.update:", error);
        res.status(500).json({
            success: false,
...
export const remove = async (req, res) => {
    try {
        ...
        const item = await FemininityGazebo.findByIdAndDelete(id);
        ...
        await addAdminAction(user._id, `Удалил(а) запись из Беседки женственности: "${item.title}"`);
        ...
        console.log("Ошибка в FemininityGazeboController.remove:", error);
        res.status(500).json({
            success: false,
