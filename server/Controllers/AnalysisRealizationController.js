    ...
    try {
        ...
        const normalizedContent = Array.isArray(content) ? content.map((item) => {
            ...
            if (item?.video?.mainUrl || item?.video?.reserveUrl) {
                return {
                    video: { mainUrl: item?.video?.mainUrl || '', reserveUrl: item?.video?.reserveUrl || '', duration: Number.isFinite(item?.video?.duration) ? item.video.duration : 0, points: Number.isFinite(item?.video?.points) ? item.video.points : 0 },
                };
            }
        ...
        const item = new AnalysisRealization({
        ...
        await item.save();
        await addAdminAction(user._id, `Создал(а) запись в Разборы - Реализация: "${title}"`);

        res.status(201).json({ success: true, data: item, message: "Запись успешно создана" });
    } catch (error) {
        console.log("Ошибка в AnalysisRealizationController.create:", error);
        res.status(500).json({ success: false, message: "Ошибка при создании записи", error: error.message });
    }
};
...
export const getAll = async (req, res) => {
    try {
        ...
        const items = await AnalysisRealization.find(filter).sort({ order: 1, createdAt: -1 });
        ...
        console.log("Ошибка в AnalysisRealizationController.getAll:", error);
...
export const getById = async (req, res) => {
    try {
        const { id } = req.params;
        const item = await AnalysisRealization.findById(id);
        ...
        console.log("Ошибка в AnalysisRealizationController.getById:", error);
        res.status(500).json({ success: false, message: "Ошибка при получении записи", error: error.message });
    }
};
...
export const update = async (req, res) => {
    try {
        ...
        const item = await AnalysisRealization.findByIdAndUpdate(id, updateData, { new: true, runValidators: true });

        if (!item) {
            return res.status(404).json({ success: false, message: "Запись не найдена" });
        }

        await addAdminAction(user._id, `Обновил(а) запись в Разборы - Реализация: "${item.title}"`);
        res.json({ success: true, data: item, message: "Запись успешно обновлена" });
    } catch (error) {
        console.log("Ошибка в AnalysisRealizationController.update:", error);
        res.status(500).json({ success: false, message: "Ошибка при обновлении записи", error: error.message });
    }
};
...
export const remove = async (req, res) => {
    try {
        const user = req.user;
        const { id } = req.params;
        const item = await AnalysisRealization.findByIdAndDelete(id);

        if (!item) {
            return res.status(404).json({ success: false, message: "Запись не найдена" });
        }

        await addAdminAction(user._id, `Удалил(а) запись из Разборы - Реализация: "${item.title}"`);
        res.json({ success: true, message: "Запись успешно удалена" });
    } catch (error) {
        console.log("Ошибка в AnalysisRealizationController.remove:", error);
        res.status(500).json({ success: false, message: "Ошибка при удалении записи", error: error.message });
    }
};
