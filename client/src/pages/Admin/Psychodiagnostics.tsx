...
export const PsychodiagnosticsAdmin = () => {
    ...
    const handleCreate = () => { navigate('/admin/psychodiagnostics/create'); };
    const handleEdit = (item: any) => { navigate(`/admin/psychodiagnostics/edit/${item._id}`); };
    const handleDelete = async (item: any) => {
        if (!confirm('Вы уверены, что хотите удалить эту запись?')) return;
        try { await api.delete(`/api/psychodiagnostics/${item._id}`); toast.success('Запись удалена'); fetchItems(); }
        ...
        <AdminLayout>
            <div className="space-y-6">
                <div className="flex justify-between items-center">
                    <h1 className="text-3xl font-bold text-gray-900">Психодиагностика</h1>
