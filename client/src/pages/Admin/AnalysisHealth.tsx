...
export const AnalysisHealthAdmin = () => {
    ...
    const handleCreate = () => { navigate('/admin/analysis-health/create'); };
    const handleEdit = (item: any) => { navigate(`/admin/analysis-health/edit/${item._id}`); };
    const handleDelete = async (item: any) => {
        if (!confirm('Вы уверены, что хотите удалить эту запись?')) return;
        try { await api.delete(`/api/analysis-health/${item._id}`); toast.success('Запись удалена'); fetchItems(); }
        ...
        <AdminLayout>
            <div className="space-y-6">
                <div className="flex justify-between items-center">
                    <h1 className="text-3xl font-bold text-gray-900">Разборы - Здоровье</h1>
