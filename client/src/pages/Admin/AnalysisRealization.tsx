...
export const AnalysisRealizationAdmin = () => {
    ...
    const fetchItems = async () => {
        try { const response = await api.get('/api/analysis-realization?admin=1'); setItems(response.data.data); }
        catch (error: any) { toast.error('Ошибка загрузки данных'); }
    };

    const handleCreate = () => { navigate('/admin/analysis-realization/create'); };
    const handleEdit = (item: any) => { navigate(`/admin/analysis-realization/edit/${item._id}`); };
    const handleDelete = async (item: any) => {
        if (!confirm('Вы уверены, что хотите удалить эту запись?')) return;
        try { await api.delete(`/api/analysis-realization/${item._id}`); toast.success('Запись удалена'); fetchItems(); }
        ...
        <AdminLayout>
            <div className="space-y-6">
                <div className="flex justify-between items-center">
                    <h1 className="text-3xl font-bold text-gray-900">Разборы - Реализация</h1>
