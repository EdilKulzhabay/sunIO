...
export const create = async (req, res) => {
    try {
        ...
        await healthLab.save();

        await addAdminAction(user._id, `Создал(а) запись в Лаборатории здоровья: "${title}"`);
        ...
        console.log("Ошибка в HealthLabController.create:", error);
        res.status(500).json({
            success: false,
...
export const getAll = async (req, res) => {
    try {
        ...
        const items = await HealthLab.find(filter).sort({ order: 1, createdAt: -1 });
        ...
        console.log("Ошибка в HealthLabController.getAll:", error);
        res.status(500).json({
            success: false,
...
export const getById = async (req, res) => {
    try {
        ...
        const item = await HealthLab.findById(id);
        ...
        console.log("Ошибка в HealthLabController.getById:", error);
        res.status(500).json({
            success: false,
...
export const update = async (req, res) => {
    try {
        ...
        const item = await HealthLab.findByIdAndUpdate(
            id,
            updateData,
        ...
        await addAdminAction(user._id, `Обновил(а) запись в Лаборатории здоровья: "${item.title}"`);
        ...
        console.log("Ошибка в HealthLabController.update:", error);
        res.status(500).json({
            success: false,
...
export const remove = async (req, res) => {
    try {
        ...
        const item = await HealthLab.findByIdAndDelete(id);
        ...
        await addAdminAction(user._id, `Удалил(а) запись из Лаборатории здоровья: "${item.title}"`);
        ...
        console.log("Ошибка в HealthLabController.remove:", error);
        res.status(500).json({
            success: false,
