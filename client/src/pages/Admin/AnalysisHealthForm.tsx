...
export const AnalysisHealthForm = () => {
    ...
    const fetchItem = async () => {
        try {
            const response = await api.get(`/api/analysis-health/${id}`);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (id) { await api.put(`/api/analysis-health/${id}`, payload); toast.success('Запись обновлена'); } else { await api.post('/api/analysis-health', payload); toast.success('Запись создана'); }
            navigate('/admin/analysis-health');
    ...
    return (
        <AdminLayout>
            <div className="max-w-6xl mx-auto space-y-6">
                <div className="flex items-center gap-4"><button onClick={() => navigate('/admin/analysis-health')} className="p-2 hover:bg-gray-100 rounded-lg transition-colors"><ArrowLeft size={24} /></button><h1 className="text-3xl font-bold text-gray-900">{id ? 'Редактировать запись' : 'Добавить запись'} - Разборы - Здоровье</h1></div>
